...
const { betamenu } = require('./src/betamenu')
...
const { betamenu } = require('./database/betamenu')
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'BETA'//Nama Bot
                        const ownerName = 'lucas'//Nama Owner
           ...
           break
				...
				case 'remover':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Se ferrouKJKKK', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
            ...
            break
				...
				case 'audio':
					...
					break
				case 'simi':
					if (args.length < 1) return reply('oi, cade o texto?')
					teks = body.slice(5)
